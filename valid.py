...
def main():
    ...
    _, _, _, _, extra_val_loaders, _ = datasets.setup_loaders(args)
...
def validate(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter, save_pth=True):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        # # visualization
        # visual = output.cpu().data[0].numpy()
        # visual = visual.transpose(1, 2, 0)
        # visual = np.asarray(np.argmax(visual, axis=2), dtype=np.int)
        # #
        # visual_col = colorize_mask(visual)
        # #
        # # print(img_names)
        # #
        # # # name = name[0].split('/')[-1]
        # #
        # visual_col.save(
        #     '%s/%s_color.png' % ('/home/lmx/DG/SPSC/DPAN-V3-2/res50_dpn_visual/' + str(dataset), img_names))
